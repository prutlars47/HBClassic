...
HealComm.ALL_HEALS, HealComm.OVERTIME_HEALS, HealComm.OVERTIME_AND_BOMB_HEALS, HealComm.CHANNEL_HEALS, HealComm.DIRECT_HEALS, HealComm.HOT_HEALS, HealComm.CASTED_HEALS, HealComm.ABSORB_SHIELDS, HealComm.ALL_DATA, HealComm.BOMB_HEALS =
    ALL_HEALS, OVERTIME_HEALS, OVERTIME_AND_BOMB_HEALS, CHANNEL_HEALS, DIRECT_HEALS, HOT_HEALS, CASTED_HEALS,
    ABSORB_SHIELDS, ALL_DATA, BOMB_HEALS
...
local spellData, hotData, tempPlayerList, pendingHeals, pendingHots = HealComm.spellData, HealComm.hotData,
    HealComm.tempPlayerList, HealComm.pendingHeals, HealComm.pendingHots
...
if (not HealComm.compressGUID) then
  HealComm.compressGUID = setmetatable({}, { __index = function (tbl, guid)
    ...
    if strsub(guid, 1, 6) ~= "Player" then
      ...
      if not str then -- assert(str, "Could not encode: "..guid)
        return nil
      end
    else
...
function HealComm:GetGUIDUnitMapTable()
  if (not HealComm.protectedMap) then
    HealComm.protectedMap = setmetatable({}, { __index = function (tbl, key) return HealComm.guidToUnit[key] end, __newindex = function ()
      error(
        "This is a read only table and cannot be modified.", 2)
...
local function calculateGeneralAmount(level, amount, spellPower, spModifier, healModifier)
  ...
  if isWrath then   -- https://wowwiki-archive.fandom.com/wiki/Downranking
    penalty = min(1, max(0, (22 + (level + 5) - playerLevel) / 20))
  elseif isTBC then -- TBC added another downrank penalty
...
-- Keep track of current cast target of Efflorescence
  ...
  LoadClassData = function ()
    ...
    if isTBC or isWrath then
      spellData[Tranquility] = { _isChanneled = true, coeff = 1.145, ticks = 4, interval = 2, levels = { 30, 40, 50, 60, 70, 75, 80 }, averages = {
        { 351 * 4, 354 * 4, 356 * 4, 358 * 4, 360 * 4, 362 * 4, 365 * 4 }, { 515 * 4, 518 * 4, 521 * 4, 523 * 4, 526 * 4, 528 * 4, 531 * 4 }, { 765 * 4, 769 * 4, 772 * 4, 776 * 4, 779 * 4, 782 * 4, 786 * 4 }, { 1097 * 4, 1101 * 4, 1105 * 4, 1109 * 4, 1112 * 4, 1116 * 4, 1120 * 4 }, { 1518 * 4, 1523 * 4, 1527 * 4, 1532 * 4, 1536 * 4 }, { 2598 * 4, 2606 * 4, 2614 * 4, 2622 * 4, 2629 * 4 }, { 3035 * 4 } } }
    else
      spellData[Tranquility] = { _isChanneled = true, coeff = 1 / 3, ticks = 5, interval = 2, levels = { 30, 40, 50, 60 }, averages = {
        { 94 * 5, 95 * 5, 96 * 5, 96 * 5, 97 * 5, 97 * 5, 98 * 5 }, { 138 * 5, 139 * 5, 140 * 5, 141 * 5, 141 * 5, 142 * 5, 143 * 5 }, { 205 * 5, 206 * 5, 207 * 5, 208 * 5, 209 * 5, 210 * 5, 211 * 5 }, { 294 * 5 } } }
    end
    if isSoD then
    ...
    CalculateHealing = function (guid, spellID, unit)
      ...
      if isTBC or isWrath then
        healModifier = healModifier * (1 + talentData[HealingLight].current) *
            (1 + (talentData[Divinity].current * (UnitIsUnit(unit, "player") and 2 or 1)))
      else
      ...
      if isWrath then
        if (unitHasAura("player", AvengingWrath)) then
          healModifier = healModifier * 1.2
        end
        if (unitHasAura("player", DivinePlea)) then
          healModifier = healModifier * 0.5
        end
      end
    ...
    CalculateHealing = function (guid, spellID)
      ...
      if (spellName == GreaterHeal) then
        ...
        spellPower = spellPower *
            ((spellData[spellName].coeff * (isWrath and 1.88 or 1)) * (1 + (talentData[EmpoweredHealing].current * 2)))
        -- Flash Heal
      elseif (spellName == FlashHeal) then
        spellPower = spellPower *
            ((spellData[spellName].coeff * (isWrath and 1.88 or 1)) * (1 + talentData[EmpoweredHealing].current))
        -- Binding Heal
...
if (playerClass == "SHAMAN") then
  LoadClassData = function ()
    ...
    if isSoD then
      hotData[HealingRain] = { interval = 1, ticks = 10, coeff = 10 * 0.063, levels = { nil }, averages =
          generateSODAverages(29.888200, 1.5, 0.690312, 0.136267) }
      hotData[Riptide] = { interval = 3, ticks = 5, coeff = 0.215, levels = { nil }, averages = generateSODAverages(
        29.888200, 1.2, 0.690312, 0.136267) }
    end
    spellData[ChainHeal] = { coeff = 2.5 / 3.5, levels = { 40, 46, 54, 61, 68, 74, 80 }, averages = {
      { avg(320, 368), avg(322, 371), avg(325, 373), avg(327, 376), avg(330, 378), avg(332, 381) }, { avg(405, 465), avg(407, 468), avg(410, 471), avg(413, 474), avg(416, 477), avg(419, 479) }, { avg(551, 629), avg(554, 633), avg(557, 636), avg(560, 639), avg(564, 643), avg(567, 646) }, { avg(605, 691), avg(608, 695), avg(612, 699), avg(616, 703), avg(620, 707), avg(624, 710) }, { avg(826, 942), avg(829, 946), avg(833, 950), avg(837, 954), avg(841, 958), avg(845, 961) }, { avg(906, 1034), avg(909, 1038), avg(913, 1042), avg(917, 1046), avg(921, 1050), avg(925, 1053) }, { avg(1055, 1205) } } }
    spellData[HealingWave] = { levels = { 1, 6, 12, 18, 24, 32, 40, 48, 56, 60, 63, 70, 75, 80 }, averages = {
      { avg(34, 44), avg(34, 45), avg(35, 46), avg(36, 47) }, { avg(64, 78), avg(65, 79), avg(66, 80), avg(67, 81), avg(68, 82), avg(69, 83) }, { avg(129, 155), avg(130, 157), avg(132, 158), avg(133, 160), avg(135, 161), avg(136, 163) }, { avg(268, 316), avg(270, 319), avg(272, 321), avg(274, 323), avg(277, 326), avg(279, 328) }, { avg(376, 440), avg(378, 443), avg(381, 446), avg(384, 449), avg(386, 451), avg(389, 454) }, { avg(536, 622), avg(539, 626), avg(542, 629), avg(545, 632), avg(549, 636), avg(552, 639) }, { avg(740, 854), avg(743, 858), avg(747, 862), avg(751, 866), avg(755, 870), avg(759, 874) }, { avg(1017, 1167), avg(1021, 1172), avg(1026, 1177), avg(1031, 1182), avg(1035, 1186), avg(1040, 1191) }, { avg(1367, 1561), avg(1372, 1567), avg(1378, 1572), avg(1383, 1578), avg(1389, 1583), avg(1394, 1589) }, { avg(1620, 1850), avg(1625, 1856), avg(1631, 1861), avg(1636, 1867), avg(1642, 1872), avg(1647, 1878) }, { avg(1725, 1969), avg(1731, 1976), avg(1737, 1982), avg(1743, 1988), avg(1750, 1995), avg(1756, 2001) }, { avg(2134, 2436), avg(2141, 2444), avg(2148, 2451), avg(2155, 2458), avg(2162, 2465) }, { avg(2624, 2996), avg(2632, 3004), avg(2640, 3012), avg(2648, 3020), avg(2656, 3028) }, { avg(3034, 3466) } } }
    spellData[LesserHealingWave] = { coeff = 1.5 / 3.5, levels = { 20, 28, 36, 44, 52, 60, 66, 72, 77 }, averages = {
      { avg(162, 186), avg(163, 188), avg(165, 190), avg(167, 192), avg(168, 193), avg(170, 195) }, { avg(247, 281), avg(249, 284), avg(251, 286), avg(253, 288), avg(255, 290), avg(257, 292) }, { avg(337, 381), avg(339, 384), avg(342, 386), avg(344, 389), avg(347, 391), avg(349, 394) }, { avg(458, 514), avg(461, 517), avg(464, 520), avg(467, 523), avg(470, 526), avg(473, 529) }, { avg(631, 705), avg(634, 709), avg(638, 713), avg(641, 716), avg(645, 720), avg(649, 723) }, { avg(832, 928), avg(836, 933), avg(840, 937), avg(844, 941), avg(848, 945), avg(853, 949) }, { avg(1039, 1185), avg(1043, 1190), avg(1047, 1194), avg(1051, 1198), avg(1055, 1202) }, { avg(1382, 1578), avg(1387, 1583), avg(1392, 1588), avg(1397, 1593), avg(1402, 1598) }, { avg(1606, 1834), avg(1612, 1840), avg(1618, 1846), avg(1624, 1852) } } }
...
if( playerClass == "WARLOCK" ) then
	LoadClassData = function()
		...
		GetHealTargets = function(bitType, guid, spellID)
			local spellName = GetSpellInfo(spellID)
			local petGUID = UnitGUID("pet")
			if spellName == DrainLife then 				return compressGUID[playerGUID]
			else
				return petGUID and compressGUID[petGUID]
			end
		end
		...
		CalculateHealing = function(guid, spellID)
			...
			if spellName == DrainLife then 				healModifier = healModifier * (1 + talentData[ShadowMastery].current)
			elseif spellName == HealthFunnel then 				healModifier = healModifier * (1 + talentData[ImpHealthFunnel].current)
			end
...
HealComm.healingModifiers = HealComm.healingModifiers or { [28776] = 0.10, -- Necrotic Poison
...
HealComm.healingStackMods = HealComm.healingStackMods or {
  ...
  [38770] = function (stacks) return 1 - stacks * 0.05 end, -- Dark Touched
  [45347] = function (stacks) return 1 - stacks * 0.05 end, -- Nether Portal - Dominance
  [30423] = function (stacks) return 1 - stacks * 0.01 end, -- Focused Will
...
function HealComm:UNIT_AURA(unit)
  ...
  wipe(alreadyAdded)
  if (unit == "player") then
    if unitHasAura("player", 10060) and not (isWrath or isTBC) then
      -- Power Infusion
...
local function loadHealList(pending, amount, stack, endTime, ticksLeft, ...)
  ...
  if (amount ~= -1 and amount ~= "-1") then
    ...
    for i = 1, select("#", ...), 2 do
      local guid = select(i, ...)
      local decompGUID = guid and decompressGUID[guid]
      amount = not pending.hasVariableTicks and tonumber((select(i + 1, ...))) or
...
function HealComm:COMBAT_LOG_EVENT_UNFILTERED(...)
  ...
  -- We only handle own group units
...
local PlayerTargetSpells = { [GetSpellInfo(689)] = true, -- Drain Life
...
function HealComm:UNIT_SPELLCAST_START(unit, cast, spellID)
  ...
  if (isTBC or isWrath) and not castUnit and PlayerTargetSpells[spellName] then
    ...
    castUnit = "player"
  elseif glyphCache[55440] and not castUnit and PlayerTargetSpells[spellName] then
  ...
  -- only here until I compress/decompress npcs
