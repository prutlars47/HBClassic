...
local major = "LibHealComm-2.0-Classic"
local minor = 201
...
local spellRankTableData = { [1] = { 774, 8936, 5185, 740, 635, 19750, 139, 2060, 596, 2061, 2054, 2050, 1064, 331, 8004, 136, 755, 689, 746, 33763, 32546, 37563, 48438, 61295, 51945, 50464, 47757, 408120, 408124, 402277, 415236, 415240, 412510, 401417, 417057, 425268, 436937, 408247, 417149, 408521, 436516, 417146, 417147 }, [2] = { 1058, 8938, 5186, 8918, 639, 19939, 6074, 10963, 996, 9472, 2055, 2052, 10622, 332, 8008, 3111, 3698, 699, 1159, 53248, 61299, 51990, 48450, 52986, 48119, 417058, 425269, 436938 }, [3] = { 1430, 8939, 5187, 9862, 647, 19940, 6075, 10964, 10960, 9473, 6063, 2053, 10623, 547, 8010, 3661, 3699, 709, 3267, 53249, 61300, 51997, 48451, 52987, 48120, 417059, 425270, 436939 }, [4] = { 2090, 8940, 5188, 9863, 1026, 19941, 6076, 10965, 10961, 9474, 6064, 913, 10466, 3662, 3700, 7651, 3268, 25422, 53251, 61301, 51998, 52988, 417060, 425271, 436940 }, [5] = { 2091, 8941, 5189, 1042, 19942, 6077, 22009, 25314, 25316, 10915, 939, 10467, 13542, 11693, 11699, 7926, 25423, 26983, 51999, 417061, 425272, 436942 }, [6] = { 3627, 9750, 6778, 3472, 19943, 6078, 10916, 959, 10468, 13543, 11694, 11700, 7927, 23569, 24412, 25210, 25308, 52000, 55458, 48446, 417062, 425273, 436943 }, [7] = { 8910, 9856, 8903, 10328, 10927, 10917, 8005, 13544, 11695, 10838, 27137, 25213, 25420, 27219, 55459, 48447, 48072, 417063, 425274, 436944 }, [8] = { 9839, 9857, 9758, 10329, 10928, 10395, 10839, 23568, 24413, 25233, 27259, 27220, 27046, 48784, 49275, 48062, 417064, 425275, 436945 }, [9] = { 9840, 9858, 9888, 25292, 10929, 10396, 18608, 25235, 48785, 49276, 48063, 48989, 47856, 47857, 417065, 425276, 436946 }, [10] = { 9841, 9889, 25315, 25357, 18610, 23567, 24414, 26980, 27135, 48070, 48990, 417066, 425277 }, [11] = { 25299, 25297, 30020, 27136, 25221, 25391, 27030, 48442, 48071, 417068 }, [12] = { 26981, 26978, 25222, 25396, 27031, 48781, 48443 }, [13] = { 26982, 26979, 48782, 49272, 48067, 45543 }, [14] = { 49273, 48377, 48440, 48068, 51827 }, [15] = { 48378, 48441, 45544 }, [16] = { 51803 } }
local SpellIDToRank = {}
...
if (not HealComm.tableCache) then
  HealComm.tableCache = setmetatable(
    {},
    { __mode = "k" }
  )
...
function HealComm:GetGUIDUnitMapTable()
  if (not HealComm.protectedMap) then
    HealComm.protectedMap = setmetatable(
      {},
      { __index = function (tbl, key) return HealComm.guidToUnit[key] end, __newindex = function () error("This is a read only table and cannot be modified.", 2) end, __metatable = false }
    )
  end
...
function HealComm:GetTimeframeHealAmount(guid, bitFlag, startTime, time, ignoreGUID, srcGUID)
  ...
  if startTime and time and (startTime > time) then
    return
  end
  for _, tbl in pairs({ pendingHeals, pendingHots }) do
    for casterGUID, spells in pairs(tbl) do
          ...
          if (pending.bitType and bit.band(pending.bitType, bitFlag) > 0) then
            for i = 1, #(pending), 5 do
              ...
              if (not guid or targetGUID == guid) then
                ...
                if ((pending.bitType == DIRECT_HEALS or pending.bitType == BOMB_HEALS) and (not time or not startTime or (startTime <= endTime and endTime <= time))) then
...
function HealComm:GetHealAmount(guid, bitFlag, time, casterGUID)
  ...
  if (casterGUID and (pendingHeals[casterGUID] or pendingHots[casterGUID])) then
    amount = filterData(pendingHeals[casterGUID], guid, bitFlag, time) +
        filterData(pendingHots[casterGUID], guid, bitFlag, time)
...
if (playerClass == "DRUID") then
  LoadClassData = function ()
    ...
    local TreeofLife = GetSpellInfo(33891) or GetSpellInfo(439733) or "Tree of Life"
    local Efflorescence = GetSpellInfo(417146) or "Efflorescence"
    hotData[Regrowth] = { interval = 3, ticks = 7, coeff = 0.5, levels = { 12, 18, 24, 30, 36, 42, 48, 54, 60, 65, 71, 77 }, averages = { 98, 175, 259, 343, 427, 546, 686, 861, 1064, 1274, 1792, 2345 } }
    ...
    if isSoD then
      hotData[Lifebloom] = { interval = 1, ticks = 7, coeff = 7 * 0.051, dhCoeff = 0.274, levels = { nil }, averages = generateSODAverages(38.94983, 0.04 * 7, 0.606705, 0.16778), bomb = generateSODAverages(38.94983, 0.57, 0.606705, 0.16778) }
      hotData[WildGrowth] = { interval = 1, ticks = 7, coeff = 7 * 0.061, levels = { nil }, averages = generateSODAverages(38.94983, 0.34 * 7, 0.606705, 0.16778) }
      hotData[Efflorescence] = { interval = 1, ticks = 15, coeff = 7 * 0.061, levels = { nil }, averages = generateSODAverages(38.94983, 0.21, 0.606705, 0.16778) }
    end
    spellData[HealingTouch] = { levels = { 1, 8, 14, 20, 26, 32, 38, 44, 50, 56, 60, 62, 69 }, averages = { { avg(37, 51), avg(37, 52), avg(38, 53), avg(39, 54), avg(40, 55) }, { avg(88, 112), avg(89, 114), avg(90, 115), avg(91, 116), avg(93, 118), avg(94, 119) }, { avg(195, 243), avg(196, 245), avg(198, 247), avg(200, 249), avg(202, 251), avg(204, 253) }, { avg(363, 445), avg(365, 448), avg(368, 451), avg(371, 454), avg(373, 456), avg(376, 459) }, { avg(572, 694), avg(575, 698), avg(579, 701), avg(582, 705), avg(586, 708), avg(589, 712) }, { avg(742, 894), avg(746, 898), avg(750, 902), avg(754, 906), avg(758, 910), avg(762, 914) }, { avg(936, 1120), avg(940, 1125), avg(945, 1129), avg(949, 1134), avg(954, 1138), avg(958, 1143) }, { avg(1199, 1427), avg(1204, 1433), avg(1209, 1438), avg(1214, 1443), avg(1219, 1448), avg(1225, 1453) }, { avg(1516, 1796), avg(1521, 1802), avg(1527, 1808), avg(1533, 1814), avg(1539, 1820), avg(1545, 1826) }, { avg(1890, 2230), avg(1896, 2237), avg(1903, 2244), avg(1909, 2250), avg(1916, 2257), avg(1923, 2263) }, { avg(2267, 2677), avg(2274, 2685), avg(2281, 2692), avg(2288, 2699), avg(2296, 2707), avg(2303, 2714) }, { avg(2364, 2790), avg(2371, 2798), avg(2378, 2805), avg(2386, 2813), avg(2393, 2820), avg(2401, 2827) }, { avg(2707, 3197), avg(2715, 3206) } } }
    spellData[Regrowth] = { coeff = 0.5 * (2 / 3.5), levels = hotData[Regrowth].levels, averages = { { avg(84, 98), avg(85, 100), avg(87, 102), avg(89, 104), avg(91, 106), avg(93, 107) }, { avg(164, 188), avg(166, 191), avg(169, 193), avg(171, 196), avg(174, 198), avg(176, 201) }, { avg(240, 274), avg(243, 278), avg(246, 281), avg(249, 284), avg(252, 287), avg(255, 290) }, { avg(318, 360), avg(321, 364), avg(325, 368), avg(328, 371), avg(332, 375), avg(336, 378) }, { avg(405, 457), avg(409, 462), avg(413, 466), avg(417, 470), avg(421, 474), avg(425, 478) }, { avg(511, 575), avg(515, 580), avg(520, 585), avg(525, 590), avg(529, 594), avg(534, 599) }, { avg(646, 724), avg(651, 730), avg(656, 735), avg(661, 740), avg(667, 746), avg(672, 751) }, { avg(809, 905), avg(815, 911), avg(821, 917), avg(827, 923), avg(833, 929), avg(839, 935) }, { avg(1003, 1119), avg(1009, 1126), avg(1016, 1133), avg(1023, 1140), avg(1030, 1147), avg(1037, 1153) }, { avg(1215, 1355), avg(1222, 1363), avg(1230, 1371), avg(1238, 1379), avg(1245, 1386), avg(1253, 1394) }, { avg(1710, 1908), avg(1720, 1919), avg(1731, 1930), avg(1742, 1941), avg(1753, 1952), avg(1764, 1962) }, { avg(2234, 2494), avg(2241, 2502), avg(2249, 2510), avg(2257, 2518) } } }
    spellData[Tranquility] = { _isChanneled = true, coeff = 1 / 3, ticks = 5, interval = 2, levels = { 30, 40, 50, 60 }, averages = { { 94 * 5, 95 * 5, 96 * 5, 96 * 5, 97 * 5, 97 * 5, 98 * 5 }, { 138 * 5, 139 * 5, 140 * 5, 141 * 5, 141 * 5, 142 * 5, 143 * 5 }, { 205 * 5, 206 * 5, 207 * 5, 208 * 5, 209 * 5, 210 * 5, 211 * 5 }, { 294 * 5 } } }
    if isSoD then
      spellData[Nourish] = { coeff = 0.357, levels = { nil }, averages = generateSODAverages(38.94983, avg(1.61, 1.89), 0.606705, 0.16778) }
    end
    ...
    itemSetsData["T1 Resto"] = { 226650, 226645, 226649, 226648, 226646, 226644, 221785, 232094, 232095, 232096, 232097, 232098, 232099, 232100, 232101 }
    itemSetsData["T2 Resto"] = { 231230, 231231, 231232, 231233, 231234, 231235, 231236, 231237 }
    ...
    GetHealTargets = function (bitType, guid, spellID)
      ...
      if (spellName == Tranquility) then
        ...
        return targets
      elseif (isSoD and spellName == Efflorescence) then
        if (guidToGroup[efflorescenceTargetGUID] ~= guidToGroup[guid]) then
          return nil
        end
      end
    ...
    CalculateHotHealing = function (guid, spellID)
      ...
      if (spellName == Rejuvenation) then
        ...
        if (equippedSetCache["Stormrage"] >= 8) then
          healAmount = healAmount + (healAmount / ticks)
          duration = duration + 3
          ticks = ticks + 1
        end
        totalTicks = ticks
      ...
      elseif (spellName == WildGrowth) then
        ...
        table.wipe(wgTicks)
        local tickModifier = equippedSetCache["Lasherweave"] >= 2 and 0.7 or 1
        ...
        if equippedSetCache["T2 Resto"] >= 6 then
          healModifier = healModifier * 1.1
        end
        if unitHasAura("Player", 439773) then
          healModifier = healModifier * 1.6
        elseif unitHasAura("Player", 24858) then
          healModifier = healModifier * 1.5
        end
        return HOT_HEALS, wgTicks, hotData[spellName].ticks, hotData[spellName].interval, nil, true
      elseif (isSoD and spellName == Efflorescence) then
        local _, _, _, _, _, expiration = unitHasAura("player", Efflorescence)
    ...
    CalculateHealing = function (guid, spellID)
      ...
      elseif (spellName == Nourish) then
        ...
        spellPower = spellPower * (spellData[spellName].coeff + talentData[EmpoweredTouch].current)
      ...
      elseif (spellName == Tranquility) then
        healModifier = healModifier + talentData[Genesis].current
        if equippedSetCache["T1 Resto"] >= 6 then
          healModifier = healModifier * 2
        end
...
if (playerClass == "PALADIN") then
  LoadClassData = function ()
    ...
    local blessings = { [19977] = { [HolyLight] = 210, [FlashofLight] = 60 }, [19978] = { [HolyLight] = 300, [FlashofLight] = 85 }, [19979] = { [HolyLight] = 400, [FlashofLight] = 115 }, [25890] = { [HolyLight] = 400, [FlashofLight] = 115 }, [27144] = { [HolyLight] = 580, [FlashofLight] = 185 }, [27145] = { [HolyLight] = 580, [FlashofLight] = 185 } }
    AuraHandler = function (unit, guid)
    ...
    CalculateHealing = function (guid, spellID, unit)
      ...
      if (hasDivineFavor or GetSpellCritChance(2) >= 100) then
        healAmount = healAmount * (1.5 * (1 + talentData[TouchedbytheLight].current))
      end
      return DIRECT_HEALS, ceil(healAmount)
    end
  end
...
if (playerClass == "PRIEST") then
  LoadClassData = function ()
    ...
    local Renewal = GetSpellInfo(37563) and "37563"
    local Penance = GetSpellInfo(53007) or GetSpellInfo(402174) or "Penance"
    ...
    spellData[FlashHeal] = { coeff = 1.5 / 3.5, levels = { 20, 26, 32, 38, 44, 50, 56, 61, 67, 73, 79 }, averages = { { avg(193, 237), avg(194, 239), avg(196, 241), avg(198, 243), avg(200, 245), avg(202, 247) }, { avg(258, 314), avg(260, 317), avg(262, 319), avg(264, 321), avg(266, 323), avg(269, 325) }, { avg(327, 393), avg(329, 396), avg(332, 398), avg(334, 401), avg(337, 403), avg(339, 406) }, { avg(400, 478), avg(402, 481), avg(405, 484), avg(408, 487), avg(411, 490), avg(414, 492) }, { avg(518, 616), avg(521, 620), avg(524, 623), avg(527, 626), avg(531, 630), avg(534, 633) }, { avg(644, 764), avg(647, 768), avg(651, 772), avg(655, 776), avg(658, 779), avg(662, 783) }, { avg(812, 958), avg(816, 963), avg(820, 967), avg(824, 971), avg(828, 975), avg(833, 979) }, { avg(913, 1059), avg(917, 1064), avg(922, 1069), avg(927, 1074), avg(931, 1078) }, { avg(1101, 1279), avg(1106, 1285), avg(1111, 1290), avg(1116, 1295), avg(1121, 1300) }, { avg(1578, 1832), avg(1586, 1840), avg(1594, 1848), avg(1602, 1856), avg(1610, 1864), avg(1618, 1872) }, { avg(1887, 2193), avg(1896, 2203) } } }
    spellData[GreaterHeal] = { coeff = 3 / 3.5, levels = { 40, 46, 52, 58, 60, 63, 68, 73, 78 }, averages = { { avg(899, 1013), avg(904, 1019), avg(909, 1024), avg(914, 1029), avg(919, 1034), avg(924, 1039) }, { avg(1149, 1289), avg(1154, 1295), avg(1160, 1301), avg(1166, 1307), avg(1172, 1313), avg(1178, 1318) }, { avg(1437, 1609), avg(1443, 1616), avg(1450, 1623), avg(1456, 1629), avg(1463, 1636), avg(1470, 1642) }, { avg(1798, 2006), avg(1805, 2014), avg(1813, 2021), avg(1820, 2029), avg(1828, 2036), avg(1835, 2044) }, { avg(1966, 2194), avg(1974, 2203), avg(1982, 2211), avg(1990, 2219), avg(1998, 2227), avg(2006, 2235) }, { avg(2074, 2410), avg(2082, 2419), avg(2090, 2427), avg(2099, 2436), avg(2107, 2444) }, { avg(2396, 2784), avg(2405, 2794), avg(2414, 2803), avg(2423, 2812), avg(2433, 2822) }, { avg(3395, 3945), avg(3408, 3959), avg(3421, 3972), avg(3434, 3985), avg(3447, 3998) }, { avg(3950, 4590), avg(3965, 4606), avg(3980, 4621) } } }
    spellData[Heal] = { coeff = 3 / 3.5, levels = { 16, 22, 28, 34 }, averages = { { avg(295, 341), avg(297, 344), avg(299, 346), avg(302, 349), avg(304, 351), avg(307, 353) }, { avg(429, 491), avg(432, 495), avg(435, 498), avg(438, 501), avg(441, 504), avg(445, 507) }, { avg(566, 642), avg(570, 646), avg(574, 650), avg(578, 654), avg(582, 658), avg(586, 662) }, { avg(712, 804), avg(716, 809), avg(721, 813), avg(725, 818), avg(730, 822), avg(734, 827) } } }
    spellData[LesserHeal] = { levels = { 1, 4, 10 }, averages = { { avg(46, 56), avg(46, 57), avg(47, 58) }, { avg(71, 85), avg(72, 87), avg(73, 88), avg(74, 89), avg(75, 90), avg(76, 91) }, { avg(135, 157), avg(136, 159), avg(138, 161), avg(139, 162), avg(141, 164), avg(143, 165) } } }
    spellData[PrayerofHealing] = { coeff = 3 / 3.5 / 3, levels = { 30, 40, 50, 60, 60, 68, 76 }, averages = { { avg(301, 321), avg(302, 323), avg(303, 324), avg(304, 325), avg(306, 327), avg(307, 328), avg(308, 329), avg(310, 331), avg(311, 332), avg(312, 333) }, { avg(444, 472), avg(445, 474), avg(447, 476), avg(448, 477), avg(450, 479), avg(452, 480), avg(453, 482), avg(455, 484), avg(456, 485), avg(458, 487) }, { avg(657, 695), avg(659, 697), avg(661, 699), avg(663, 701), avg(665, 703), avg(667, 705), avg(669, 707), avg(671, 709), avg(673, 711), avg(675, 713) }, { avg(939, 991), avg(941, 994), avg(943, 996), avg(946, 999), avg(948, 1001), avg(951, 1003), avg(953, 1006), avg(955, 1008), avg(958, 1011), avg(960, 1013) }, { avg(997, 1053), avg(999, 1056), avg(1002, 1058), avg(1004, 1061), avg(1007, 1063), avg(1009, 1066), avg(1012, 1068), avg(1014, 1071), avg(1017, 1073), avg(1019, 1076) }, { avg(1246, 1316), avg(1248, 1319), avg(1251, 1322), avg(1254, 1325), avg(1257, 1328), avg(1260, 1330), avg(1262, 1333), avg(1265, 1336) }, { avg(2091, 2209), avg(2095, 2214), avg(2100, 2219), avg(2105, 2224), avg(2109, 2228) } } }
    spellData[BindingHeal] = { coeff = 1.5 / 3.5, levels = { 64, 72, 78 }, averages = { { avg(1042, 1338), avg(1043, 1340), avg(1045, 1342), avg(1047, 1344), avg(1049, 1346), avg(1051, 1348), avg(1053, 1350), avg(1055, 1352) }, { avg(1619, 2081), avg(1622, 2084), avg(1625, 2087), avg(1628, 2090), avg(1631, 2093) }, { avg(1952, 2508), avg(1955, 2512), avg(1959, 2516) } } }
    if isSoD then
    ...
    --itemSetsData["T1 Resto"] = {226571, 226573, 226577, 226572, 226576, 226574, 226575, 226578, 232182, 232181,232180,232179,232178,232177,232176,232175}  --Core Forged
    --itemSetsData["T2 Resto"] = {231159, 231155, 231156, 231157, 231158, 231160, 231161, 231162 }
    -- 467586 S03 - Item - T2 - Priest - Healer 6P Bonus: Circle of Healing and Penance also place a heal over time effect on their targets that heals for 25% as much over 15 sec.
    ...
    CalculateHotHealing = function (guid, spellID)
      ...
      if (spellName == Renew or spellName == GreaterHealHot) then
        ...
        spellPower = spellPower * (1 * (1 + talentData[EmpoweredRenew].current))
...
if (playerClass == "SHAMAN") then
  LoadClassData = function ()
    ...
    local Riptide = GetSpellInfo(61295) or GetSpellInfo(408521) or "Riptide"
    local Earthliving = GetSpellInfo(52000) or "Earthliving"
    local HealingRain = GetSpellInfo(415236) or "Healing Rain"
    hotData[Riptide] = { interval = 3, ticks = 5, coeff = 0.5, levels = { 60, 70, 75, 80 }, averages = { 665, 885, 1435, 1670 } }
    hotData[Earthliving] = { interval = 3, ticks = 4, coeff = 0.364, levels = { 30, 40, 50, 60, 70, 80 }, averages = { 116, 160, 220, 348, 456, 652 } }
    if isSoD then
      hotData[HealingRain] = { interval = 1, ticks = 10, coeff = 10 * 0.063, levels = { nil }, averages = generateSODAverages(29.8882, 1.5, 0.690312, 0.136267) }
      hotData[Riptide] = { interval = 3, ticks = 5, coeff = 0.215, levels = { nil }, averages = generateSODAverages(29.8882, 1.2, 0.690312, 0.136267) }
    end
    spellData[ChainHeal] = { coeff = 2.5 / 3.5, levels = { 40, 46, 54, 61, 68, 74, 80 }, averages = { { avg(320, 368), avg(322, 371), avg(325, 373), avg(327, 376), avg(330, 378), avg(332, 381) }, { avg(405, 465), avg(407, 468), avg(410, 471), avg(413, 474), avg(416, 477), avg(419, 479) }, { avg(551, 629), avg(554, 633), avg(557, 636), avg(560, 639), avg(564, 643), avg(567, 646) }, { avg(605, 691), avg(608, 695), avg(612, 699), avg(616, 703), avg(620, 707), avg(624, 710) }, { avg(826, 942), avg(829, 946), avg(833, 950), avg(837, 954), avg(841, 958), avg(845, 961) }, { avg(906, 1034), avg(909, 1038), avg(913, 1042), avg(917, 1046), avg(921, 1050), avg(925, 1053) }, { avg(1055, 1205) } } }
    spellData[HealingWave] = { levels = { 1, 6, 12, 18, 24, 32, 40, 48, 56, 60, 63, 70, 75, 80 }, averages = { { avg(34, 44), avg(34, 45), avg(35, 46), avg(36, 47) }, { avg(64, 78), avg(65, 79), avg(66, 80), avg(67, 81), avg(68, 82), avg(69, 83) }, { avg(129, 155), avg(130, 157), avg(132, 158), avg(133, 160), avg(135, 161), avg(136, 163) }, { avg(268, 316), avg(270, 319), avg(272, 321), avg(274, 323), avg(277, 326), avg(279, 328) }, { avg(376, 440), avg(378, 443), avg(381, 446), avg(384, 449), avg(386, 451), avg(389, 454) }, { avg(536, 622), avg(539, 626), avg(542, 629), avg(545, 632), avg(549, 636), avg(552, 639) }, { avg(740, 854), avg(743, 858), avg(747, 862), avg(751, 866), avg(755, 870), avg(759, 874) }, { avg(1017, 1167), avg(1021, 1172), avg(1026, 1177), avg(1031, 1182), avg(1035, 1186), avg(1040, 1191) }, { avg(1367, 1561), avg(1372, 1567), avg(1378, 1572), avg(1383, 1578), avg(1389, 1583), avg(1394, 1589) }, { avg(1620, 1850), avg(1625, 1856), avg(1631, 1861), avg(1636, 1867), avg(1642, 1872), avg(1647, 1878) }, { avg(1725, 1969), avg(1731, 1976), avg(1737, 1982), avg(1743, 1988), avg(1750, 1995), avg(1756, 2001) }, { avg(2134, 2436), avg(2141, 2444), avg(2148, 2451), avg(2155, 2458), avg(2162, 2465) }, { avg(2624, 2996), avg(2632, 3004), avg(2640, 3012), avg(2648, 3020), avg(2656, 3028) }, { avg(3034, 3466) } } }
    spellData[LesserHealingWave] = { coeff = 1.5 / 3.5, levels = { 20, 28, 36, 44, 52, 60, 66, 72, 77 }, averages = { { avg(162, 186), avg(163, 188), avg(165, 190), avg(167, 192), avg(168, 193), avg(170, 195) }, { avg(247, 281), avg(249, 284), avg(251, 286), avg(253, 288), avg(255, 290), avg(257, 292) }, { avg(337, 381), avg(339, 384), avg(342, 386), avg(344, 389), avg(347, 391), avg(349, 394) }, { avg(458, 514), avg(461, 517), avg(464, 520), avg(467, 523), avg(470, 526), avg(473, 529) }, { avg(631, 705), avg(634, 709), avg(638, 713), avg(641, 716), avg(645, 720), avg(649, 723) }, { avg(832, 928), avg(836, 933), avg(840, 937), avg(844, 941), avg(848, 945), avg(853, 949) }, { avg(1039, 1185), avg(1043, 1190), avg(1047, 1194), avg(1051, 1198), avg(1055, 1202) }, { avg(1382, 1578), avg(1387, 1583), avg(1392, 1588), avg(1397, 1593), avg(1402, 1598) }, { avg(1606, 1834), avg(1612, 1840), avg(1618, 1846), avg(1624, 1852) } } }
    talentData[HealingWay] = { mod = (0.25 / 3), current = 0 }
    talentData[ImpChainHeal] = { mod = 0.1, current = 0 }
    ...
    itemSetsData["T2 Resto"] = { 213205, 213204, 213198, 213203, 213202, 213201, 213200, 213199 }
    ...
    GetHealTargets = function (bitType, guid, spellID, amount)
      ...
      if (isSoD and spellName == HealingRain) then
        if (guidToGroup[healingRainTargetGUID] ~= guidToGroup[guid]) then
          return nil
        end
      end
      return compressGUID[guid]
    ...
    CalculateHealing = function (guid, spellID, unit)
      ...
      elseif (spellName == HealingWave) then
        ...
        spellPower = spellPower * ((castTime / 3.5) + talentData[TidalWaves].current)
      elseif (spellName == LesserHealingWave) then
        spellPower = spellPower + (playerCurrentRelic and lhwTotems[playerCurrentRelic] or 0)
        spellPower = spellPower * (spellData[spellName].coeff + (talentData[TidalWaves].current / 2))
      end
      healAmount = calculateGeneralAmount(spellData[spellName].levels[spellRank], healAmount, spellPower, spModifier, healModifier)
      if (GetSpellCritChance(4) >= 100) then
        healAmount = healAmount * 1.5
...
if (isSoD and playerClass == "MAGE") then
  LoadClassData = function ()
    ...
    local ChronostaticPreservation = GetSpellInfo(436516) or "Chronostatic Preservation"
    ...
    spellData[ChronostaticPreservation] = { coeff = 2 / 3.5, levels = { nil }, averages = generateSODAverages(38.258376, avg(2, 3), 0.904195, 0.161311) }
    GetHealTargets = function (bitType, guid, spellID)
      local spellName = GetSpellInfo(spellID)
    ...
    CalculateHealing = function (guid, spellID)
      ...
      if (spellName == MassRegeneration or spellName == Regeneration) then
        ...
        return CHANNEL_HEALS, math.ceil(healAmount), spellData[spellName].ticks, spellData[spellName].tickInterval
      elseif (spellName == ChronostaticPreservation) then
        spellPower = spellPower * spellData[spellName].coeff * 1.88
        healAmount = calculateGeneralAmount(spellData[spellName].levels[spellRank], healAmount, spellPower, spModifier, healModifier)
      end
      return DIRECT_HEALS, ceil(healAmount)
...
HealComm.healingModifiers = HealComm.healingModifiers or { [28776] = 0.1, [36693] = 0.55, [46296] = 0.25, [19716] = 0.25, [13737] = 0.5, [15708] = 0.5, [16856] = 0.5, [17547] = 0.5, [19643] = 0.5, [24573] = 0.5, [27580] = 0.5, [29572] = 0.5, [31911] = 0.5, [32736] = 0.5, [35054] = 0.5, [37335] = 0.5, [39171] = 0.5, [40220] = 0.5, [44268] = 0.5, [12294] = 0.5, [21551] = 0.5, [21552] = 0.5, [21553] = 0.5, [25248] = 0.5, [30330] = 0.5, [43441] = 0.5, [30843] = 0, [19434] = 0.5, [20900] = 0.5, [20901] = 0.5, [20902] = 0.5, [20903] = 0.5, [20904] = 0.5, [27065] = 0.5, [34625] = 0.25, [35189] = 0.5, [32315] = 0.5, [32378] = 0.5, [36917] = 0.5, [44534] = 0.5, [34366] = 0.75, [36023] = 0.5, [36054] = 0.5, [45885] = 0.5, [41292] = 0, [40599] = 0.5, [9035] = 0.8, [19281] = 0.8, [19282] = 0.8, [19283] = 0.8, [19284] = 0.8, [25470] = 0.8, [34073] = 0.85, [31306] = 0.25, [44475] = 0.25, [23169] = 0.5, [22859] = 0.5, [38572] = 0.5, [39595] = 0.5, [45996] = 0, [41350] = 2, [28176] = 1.2, [7068] = 0.25, [17820] = 0.25, [22687] = 0.25, [23224] = 0.25, [24674] = 0.25, [28440] = 0.25, [13583] = 0.5, [23230] = 0.5, [31977] = 1.5 }
HealComm.healingStackMods = HealComm.healingStackMods or { [25646] = function (stacks) return 1 - stacks * 0.1 end, [28467] = function (stacks) return 1 - stacks * 0.1 end, [30641] = function (stacks) return 1 - stacks * 0.05 end, [31464] = function (stacks) return 1 - stacks * 0.1 end, [36814] = function (stacks) return 1 - stacks * 0.1 end, [38770] = function (stacks) return 1 - stacks * 0.05 end, [45347] = function (stacks) return 1 - stacks * 0.05 end, [30423] = function (stacks) return 1 - stacks * 0.01 end, [45242] = function (stacks) return 1 + stacks * 0.1 end }
...
function HealComm:UNIT_AURA(unit)
  ...
  wipe(alreadyAdded)
  if (unit == "player") then
    if unitHasAura("player", 10060) then
      playerIncrease = playerIncrease * 1.2
    end
    local npsStacks = select(3, unitHasAura("player", 30422))
    if npsStacks then
      playerIncrease = playerIncrease * (1 + npsStacks * 0.05)
    end
    if unitHasAura("player", 41406) then
      playerIncrease = playerIncrease * 1.05
    end
    if unitHasAura("player", 41409) then
      playerDecrease = playerDecrease * 0.95
    end
    if unitHasAura("player", 32346) then
      playerDecrease = playerDecrease * 0.5
    end
    if unitHasAura("player", 40099) then
      playerDecrease = playerDecrease * 0.5
    end
    if unitHasAura("player", 38246) then
      playerDecrease = playerDecrease * 0.5
    end
    if unitHasAura("player", 45573) then
      playerDecrease = playerDecrease * 0.5
    end
...
local function parseHotBomb(casterGUID, wasUpdated, spellID, amount, ...)
  ...
  pending.stack = isSoD and hotPending.stack or 1
...
local eventRegistered = { SPELL_HEAL = true, SPELL_PERIODIC_HEAL = true, SPELL_AURA_REMOVED = true, SPELL_AURA_APPLIED = true, SPELL_AURA_REFRESH = true, SPELL_AURA_APPLIED_DOSE = true, SPELL_AURA_REMOVED_DOSE = true }
function HealComm:COMBAT_LOG_EVENT_UNFILTERED(...)
  ...
  elseif (eventType == "SPELL_AURA_REMOVED" and bit.band(sourceFlags, COMBATLOG_OBJECT_AFFILIATION_MINE) == COMBATLOG_OBJECT_AFFILIATION_MINE) then
    if compressGUID[destGUID] then
      ...
      if hotData[spellName] then
        if (isSoD and spellName == "Healing Rain" and guidToGroup[healingRainTargetGUID] ~= guidToGroup[destGUID]) then
          return
        end
        if (isSoD and spellName == "Efflorescence" and guidToGroup[efflorescenceTargetGUID] ~= guidToGroup[destGUID]) then
          return
        end
...
local penanceIDs = { [47540] = 47757, [53005] = 52986, [53006] = 52987, [53007] = 52988, [402174] = 402277 }
function HealComm:UNIT_SPELLCAST_SENT(unit, targetName, castGUID, spellID)
  local spellName = GetSpellInfo(spellID)
  if (unit ~= "player") then
    return
  end
  if (isSoD and spellID == 18562) then
...
local PlayerTargetSpells = { [GetSpellInfo(689)] = true, [GetSpellInfo(740)] = true, [GetSpellInfo(331)] = true }
function HealComm:UNIT_SPELLCAST_START(unit, cast, spellID)
  if (unit ~= "player") then
    return
  end
  local spellName = GetSpellInfo(spellID)
  if (isSoD and spellName == "Healing Rain") then
...
function HealComm:UNIT_SPELLCAST_SUCCEEDED(unit, cast, spellID)
  ...
  elseif spellName == GetSpellInfo(20473) then
    hasDivineFavor = nil
  end
end
...
function HealComm:CastSpell(arg, unit)
  ...
  if tonumber(arg) then
    ...
    spellID = select(7, GetSpellInfo(arg))
  end
  local spellName = GetSpellInfo(spellID)
...
function HealComm:OnInitialize()
  ...
  do
    ...
    spellData[FirstAid] = { _isChanneled = true, ticks = { 6, 6, 7, 7, 8, 8, 8, 8, 8, 8, 8, 8, 8, 8 }, interval = 1, averages = { 66, 114, 161, 301, 400, 640, 800, 1104, 1360, 2000, 2800, 3400, 4800, 5800 } }
